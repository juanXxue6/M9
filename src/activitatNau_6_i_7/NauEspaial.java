...
class PanelNau extends JPanel implements Runnable, KeyListener{   
 ...
 public void newShot() throws InterruptedException {

	 if(contador < 9) {
	 shot[contador]=  new Shot(nauPropia.getX() + 69, nauPropia.getY() - 40 , 50); 
	 contador++;

	 }else {
		// Thread.sleep(5000);
		 int coordenada = shot[8].getY();
		 for (int i = 0; i < shot.length; i++) {
			shot[i] = null;
		}
		 contador = 0;
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (e.getKeyCode()==39) { nauPropia.dreta(); }
            if (e.getKeyCode()==32) { try {
				newShot();
        ...
        public void keyReleased(KeyEvent e) {
        	 if (e.getKeyCode()==37) { nauPropia.parar();} //System.out.println("a l'esquerra"); }
             if (e.getKeyCode()==39) { nauPropia.parar(); }
        }
}
...
class Shot extends Thread{
    ...
    public Shot (int dsx, int dsy, int v ) {
        this.dsy=dsy;
        this.dsx=dsx;
        this.v=v;
        laser = new ImageIcon(Nau.class.getResource("laser.png")).getImage();
    ...
    public void printShot (Graphics g) {
        Graphics2D g2d = (Graphics2D)g;
        g2d.drawImage(this.laser, this.dsx, this.dsy, null);
      }
    
    public void moure (){
    	int dsy = 25;
    		this.dsy = this.dsy - dsy;
        }
